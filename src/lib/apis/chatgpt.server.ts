...
export async function createCritique(text: string): Promise<string | undefined> {
    return openai.createChatCompletion({
        model: 'gpt-3.5-turbo',
        messages: [
            {role: 'system', content: 'You are a copy editor, your task is to review text and provide constructive criticism in order to assist the user improve the writing.'},
